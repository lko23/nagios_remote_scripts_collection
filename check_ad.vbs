' needs:
' C:\Program Files\NSClient++\nsclient.ini: check_dc_connectivity=check_dc\check_ad.vbs
...
sub parse(txtp)
	...
	loop
  'txtp = Replace(txtp,"bestanden","nicht bestanden") ' Zum Testen
  'txtp = Replace(txtp,"hat den test services bestanden","hat den test services nicht bestanden") ' Zum Testen
  'txtp = Replace(txtp,"ksl-vw2k004 hat den test connectivity bestanden","ksl-vw2k004 hat den test connectivity nicht bestanden") ' Zum Testen
  'txtp = Replace(txtp,"ksl-vw2k005 hat den test connectivity bestanden","ksl-vw2k005 hat den test connectivity nicht bestanden") ' Zum Testen
	...
	if instr(lcase(txtp), lcase(strNotOK)) then
		...
		'pt "Resultat :" & txtp
		for loop1 = 0 to Ubound(name)-1
      'wscript.echo "Name=" & lcase(name(loop1))
      ...
      if instr(lcase(txtp), "connectivity") then
				status(loop1)="CRITICAL! Mehr Information, Befehl auf einem DC:  dcdiag /test:intersite /e"
				'Lock the variable so it can't be reset back to success. Required for multi-partition tests like 'crossrefvalidation'
				lock(loop1)=TRUE
				pt "Reset the status for test '" & name(loop1) & "' to '" & status(loop1) & "' with a lock '" & lock(loop1) & "'"
      end if
...
function cmd()
	...
	if Wscript.Arguments.Count = 0 Then
        ...
        next
      	...
      	if (instr(lcase(wscript.Arguments(loop1)), lcase("/e"))) then
          cmd = cmd  & " /e"
        end if
      	if (instr(lcase(wscript.Arguments(loop1)), lcase("/s"))) then
          cmd = cmd  & " " & lcase(wscript.Arguments(loop1))
        end if
			end if
		next
	end if
